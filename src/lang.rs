...
pub fn get_lang_map(cli: &Cli) -> HashMap<&str, String> {
    create_lang_map!(cli, {
        lang_empty_field => "질문 '{name}' 의 답이 비어있습니다. 다시 작성해주세요.",
        ...
        lang_result_name_err => "마인크래프트 이름 형식이 잘못되었어요. 마인크래프트 인게임 유저 이름을 정확하게 입력해주세요.",
        lang_result_answer_err => "잘못된 답을 입력했어요. 질문들을 다시 확인해주세요.",
        lang_result_not_exist => "해당 마인크래프트 유저를 찾지 못했어요. 마인크래프트 인게임 유저 이름을 정확하게 입력해주세요.",
        lang_result_captcha_err => "캡챠 검증에 오류가 발생했어요",
        lang_result_unknown_err => "알 수 없는 오류가 발생했어요. 관리자에게 문의해주세요.",
    })
}
